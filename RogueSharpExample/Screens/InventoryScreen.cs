...
namespace RogueSharpExample.Screens
{
    public class InventoryScreen : Screen
    {
        ...
        public InventoryScreen(int width, int height) : base("Inventory", width, height)
        {
        }
        ...
        public void Draw(RLConsole rootConsole, Inventory inventory)
        {
            ...
            if (inventory.Item.Count > 0)
            {
                for (int i = 0; i < items.Length % 26; i++)
                {
                    ...
                    console.Print(2, 2 + i, $"{(char)(97 + i)} - {item.Name}, {item.RemainingUses} uses left", RLColor.White);
                }
